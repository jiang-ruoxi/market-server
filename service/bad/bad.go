...
func (badWordsService *BadWordsService)DeleteBadWords(badWords bad.BadWords) (err error) {
	var s bad.BadWords
	err = global.MustGetGlobalDBByDBName("market").Model(&s).Debug().Where("id=?", badWords.ID).Update("is_deleted", 1).Error
	return err
}
...
func (badWordsService *BadWordsService)DeleteBadWordsByIds(ids request.IdsReq) (err error) {
	var s bad.BadWords
	err = global.MustGetGlobalDBByDBName("market").Model(&s).Debug().Where("id IN ?", ids.Ids).Updates(&bad.BadWords{IsDeleted: 1}).Error
	return err
}
...
func (badWordsService *BadWordsService)GetBadWordsInfoList(info badReq.BadWordsSearch) (list []bad.BadWords, total int64, err error) {
    ...
    // 创建db
	db := global.MustGetGlobalDBByDBName("market").Model(&bad.BadWords{}).Where("is_deleted = 0")
